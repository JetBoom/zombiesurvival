...
if CLIENT then
	SWEP.PrintName = translate.Get("medkitgod")
	SWEP.Description = translate.Get("medkit_desk")
	SWEP.Slot = 4
	SWEP.SlotPos = 0
...
function SWEP:DrawHUD()
	...
	if 0 < timeleft then
		...
		surface.SetDrawColor(255, 215, 0, 180)
		surface.DrawOutlinedRect(x, y, wid, hei)
	end

	draw.SimpleText(translate.Get("medkitgod"), "ZSHUDFontSmall", x, texty, COLOR_GOLD, TEXT_ALIGN_LEFT)

	local charges = self:GetPrimaryAmmoCount()
	if charges > 0 then
		draw.SimpleText(charges, "ZSHUDFontSmall", x + wid, texty, COLOR_GOLD, TEXT_ALIGN_RIGHT)
	else
