...
function GM:ZombieHUD()
	...
	if not self:GetWaveActive() and self:GetWave() ~= 0 then
		...
		if pl and pl:IsValid() then
			...
			if pl == MySelf then
				draw_SimpleTextBlur(translate.Format("you_will_be_x_soon", translate.Format(GAMEMODE.ZombieClasses[GAMEMODE.NextBossZombieClass].TranslationName)), "ZSHUDFont", x, y, Color(255, 50, 50), TEXT_ALIGN_CENTER)
			else
				draw_SimpleTextBlur(translate.Format("x_will_be_y_soon", pl:Name(), translate.Format(GAMEMODE.ZombieClasses[GAMEMODE.NextBossZombieClass].TranslationName)), "ZSHUDFont", x, y, COLOR_GRAY, TEXT_ALIGN_CENTER)
			end
		end
	end
end
